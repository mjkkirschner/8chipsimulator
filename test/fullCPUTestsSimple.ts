...
describe("testing a full cpu/computer integreation", () => {
    ...
    it("can halt ", (done) => {
        ...
        setTimeout(() => {
            const aReg = orderedParts.filter((x) => { return x.pointer.displayName == "OUT register" })[0].pointer;
    ...
    it("can add A and B - and send result to Out Reg, then halt ", (done) => {
        ...
        setTimeout(() => {
            ...
            const outReg = orderedParts.filter((x) => { return x.pointer.displayName == "A register" })[0].pointer;
    ...
    it("can jump conditionally ", (done) => {
        ...
        setTimeout(() => {
            const aReg = orderedParts.filter((x) => { return x.pointer.displayName == "A register" })[0].pointer;
            assert.equal(aReg.toOutputString(), "25");
            const bReg = orderedParts.filter((x) => { return x.pointer.displayName == "B register" })[0].pointer;
